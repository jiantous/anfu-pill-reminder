...
| **提醒保留到确认** | 点通知进 App 后提醒不消失，要点「已服用」或「跳过」才消，避免看一眼就当吃过了。 |
| **今日清单** | 当天要吃的药按「已错过 / 待服用 / 已完成」分组，打过卡的显示实际服用时间。临时改时间、跳过单次都支持。 |
| **复杂周期** | 每天 / 按周固定几天 / 隔 N 天 / 吃 X 天停 Y 天。可设疗程起止日期与饭前饭后。 |
| **暂停用药** | 住院、出差等可临时暂停某药，到指定日期自动恢复。暂停期内不提醒、不算入依从率、不顺延疗程。 |
| **库存管理** | 记录每种药剩多少，打卡自动扣减，低于设定值提醒续药。 |
| **依从率统计** | 圆环 + 7 天柱状图 + 月历三种视图，可切换近 7 / 30 / 90 天。复诊时给医生看比口头回忆可靠。 |
| **拍说明书自动填写** | 拍药品说明书，自动识别药名、剂量、每日次数、饭前饭后并预填表单。**完全离线**，照片不保存不上传。 |
| **服药记录导出** | 按近 7 / 30 / 90 天或全部导出 CSV 表格，注明漏服与跳过，复诊时直接给医生看。 |
| **备份** | 一键导出为单个 JSON 文件，存哪里自己定（放云盘同步目录就会自动上传）。换手机时导入即可恢复全部历史。 |
| **提醒可靠性体检** | 自动检查通知权限、精确闹钟权限、电池优化豁免，并按手机品牌给出对应设置提示。 |

v1.1.9 
新增「按小时间隔」服药时间：填开始/结束时间和间隔小时数自动生成当天时刻表，打卡晚了下一次自动顺延，次日重新对齐开始时间；
包名改为 com.nxwzj0.pillreminder，独立维护分支
按小时间隔 追加开关：打卡晚了是否自动顺延下一次？适合某些药品时间间隔非常严苛的情况，间隔严格要超过N小时才行。
